...
describe('Sessions feature tests', () => {
	...
	it('should submit an answer and return answer object', async () => {
		...
		const answerRequest = new Request(
			`http://localhost/api/sessions/${sessionId}/answers`,
			{
				method: 'POST',
				headers: {
					Authorization: `Bearer ${token}`,
	...
	it('should return 400 when question not in session', async () => {
		// создаём 2 сессии
		...
		const { sessionId: s1, questions: q1 } = await res1.json()
		const { questions: q2 } = await res2.json()

		const чужойQuestionId = q2[0].id

		const request = new Request(`http://localhost/api/sessions/${s1}/answers`, {
			method: 'POST',
			headers: {
				Authorization: `Bearer ${token}`,
				'Content-Type': 'application/json',
			},
			body: JSON.stringify({ questionId: чужойQuestionId, userAnswer: ['0'] }),
		})
